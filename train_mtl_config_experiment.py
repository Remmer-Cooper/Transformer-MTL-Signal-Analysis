...
from models.build_mtl_modeldata1 import VisionTransformerMTL
    ...
    for epoch in range(epochs):
        ...
        with tqdm(train_loader, desc=f'Epoch {epoch + 1}/{epochs} Training') as pbar:
            for X_batch, Y_mod_batch, Y_sig_batch in pbar:
                ...
                mod_correct += (torch.argmax(mod_output, dim=1) == Y_mod_batch).sum().item()
                sig_correct += (torch.argmax(sig_output, dim=1) == Y_sig_batch).sum().item()
        ...
        with tqdm(val_loader, desc=f'Epoch {epoch + 1}/{epochs} Validation') as pbar:
            with torch.no_grad():
                for X_val_batch, Y_mod_val_batch, Y_sig_val_batch in pbar:
                    X_val_batch, Y_mod_val_batch, Y_sig_val_batch = X_val_batch.to(device), Y_mod_val_batch.to(device), Y_sig_val_batch.to(device)
                    ...
                    val_mod_correct += (torch.argmax(mod_output, dim=1) == Y_mod_val_batch).sum().item()
                    val_sig_correct += (torch.argmax(sig_output, dim=1) == Y_sig_val_batch).sum().item()
...
for idx, (Csh, Cm, Fm, Cs, Fs) in enumerate(configs):
    print(f"\n=== Training Config {idx + 1}: (Csh={Csh}, Cm={Cm}, Fm={Fm}, Cs={Cs}, Fs={Fs}) ===")
    model = VisionTransformerMTL(Csh=Csh, Cm=Cm, Fm=Fm, Cs=Cs, Fs=Fs).to(device)
    history = train_model(model, train_loader, val_loader, epochs=45)

    last_train_sig_acc = history['sig_acc'][-1][0]
    last_val_sig_acc = history['sig_acc'][-1][1]
    results[f"Config_{idx+1}_(Csh={Csh},Cm={Cm},Fm={Fm},Cs={Cs},Fs={Fs})"] = {
        "train_sig_acc": last_train_sig_acc,
        "val_sig_acc": last_val_sig_acc
    }

os.makedirs("results", exist_ok=True)
with open("results/sig_accuracy_results.json", "w") as f:
    json.dump(results, f, indent=4)

print("Signal accuracy results saved")
