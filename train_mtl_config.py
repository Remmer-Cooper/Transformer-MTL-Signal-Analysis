...
from models.build_mtl_modeldata import VisionTransformerMTL
...
print(f"Processed shape: {X_processed.shape}")  # 应为 [N,3,16,16]
...
for config in configs:
    ...
    os.makedirs("weightsdata", exist_ok=True)
    for epoch in range(45):
        ...
        mod_train_acc_list.append(mod_acc)
        mod_val_acc_list.append(val_mod_acc)
        sig_train_acc_list.append(sig_acc)
        sig_val_acc_list.append(val_sig_acc)
    ...
    data = {
        "config": config,
        "modulation_train_acc": mod_train_acc_list,
        "modulation_val_acc": mod_val_acc_list,
        "signal_train_acc": sig_train_acc_list,
        "signal_val_acc": sig_val_acc_list,

    }

    os.makedirs("results_json2", exist_ok=True)
    filename = f"results_json2/config_{config[0]}_{config[1]}_{config[2]}_{config[3]}_{config[4]}.json"
    with open(filename, 'w') as f:
        json.dump(data, f, indent=4)
 # Save the model weights
    model_filename = f"weightsdata/model_{config[0]}_{config[1]}_{config[2]}_{config[3]}_{config[4]}.pth"
    torch.save(model.state_dict(), 'weightsdata/model_{config[0]}_{config[1]}_{config[2]}_{config[3]}_{config[4]}.pth')
